...
function ChangeTheme(ThemeName)
	...
	CopybaraGui.Main.BackgroundColor3 = SelectedTheme.Background
	CopybaraGui.Main.Topbar.BackgroundColor3 = SelectedTheme.Topbar
	CopybaraGui.Main.Topbar.CornerRepair.BackgroundColor3 = SelectedTheme.Topbar
	CopybaraGui.Main.Shadow.Image.ImageColor3 = SelectedTheme.Shadow

	CopybaraGui.Main.Topbar.ChangeSize.ImageColor3 = SelectedTheme.TextColor
	CopybaraGui.Main.Topbar.Hide.ImageColor3 = SelectedTheme.TextColor
	CopybaraGui.Main.Topbar.Theme.ImageColor3 = SelectedTheme.TextColor
		...
		if Copybara.Flags[FlagName] then
			spawn(function()
				if Copybara.Flags[FlagName].Type == "ColorPicker" then
					...
					if Copybara.Flags[FlagName].CurrentValue or Copybara.Flags[FlagName].CurrentKeybind or Copybara.Flags[FlagName].CurrentOption or Copybara.Flags[FlagName].Color ~= FlagValue then
						Copybara.Flags[FlagName]:Set(FlagValue)
					end
				end
			end)
		else
			Copybara:Notify({Title = "Flag Error", Content = "Copybara was unable to find '"..FlagName.."' in the current script"})
		end
	end
end
...
local function SaveConfiguration()
	...
	local Data = {}
	for i, v in pairs(Copybara.Flags) do
...
function Hide()
	Debounce = true
	Copybara:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping K", Duration = 7})
	...
	if Settings.Discord then
		...
		if not isfile(CopybaraFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension) then
			...
			if Settings.Discord.RememberJoins then
				writefile(CopybaraFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension, "Copybara RememberJoins is true for this invite, this invite will not ask you to join again")
			end
		end
	end
	...
	if Settings.KeySystem then
		...
		if Settings.KeySettings.GrabKeyFromSite then
			for i, Key in ipairs(Settings.KeySettings.Key) do
				...
				if not Success then
					print("Copybara | "..Key.." Error " ..tostring(Response))
				end
			end
		end
		...
		if isfile(CopybaraFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) then
			for _, MKey in ipairs(Settings.KeySettings.Key) do
				if string.find(readfile(CopybaraFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension), MKey) then
					Passthrough = true
				end
			end
		end
			...
			KeyUI.Main.Input.InputBox.FocusLost:Connect(function()
				...
				if KeyFound then
					...
					if Settings.KeySettings.SaveKey then
						...
						Copybara:Notify({Title = "Key System", Content = "The key for this script has been saved successfully"})
					end
				else
		...
		function Tab:CreateInput(InputSettings)
			...
			Input.InputFrame.InputBox.FocusLost:Connect(function()
				...
				if not Success then
					...
					print("Copybara | "..InputSettings.Name.." Callback Error " ..tostring(Response))
					wait(0.5)
					Input.Title.Text = InputSettings.Name
			...
			local NewInputSettings = {}
			function NewInputSettings:Set(text)
			...
			for _, Option in ipairs(DropdownSettings.Options) do
				...
				DropdownOption.Interact.MouseButton1Click:Connect(function()
					...
					if not Success then
						...
						print("Copybara | "..DropdownSettings.Name.." Callback Error " ..tostring(Response))
						wait(0.5)
						Dropdown.Title.Text = DropdownSettings.Name
						TweenService:Create(Dropdown, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
				...
				elseif KeybindSettings.CurrentKeybind ~= nil and (input.KeyCode == Enum.KeyCode[KeybindSettings.CurrentKeybind] and not processed) then
					...
					if not KeybindSettings.HoldToInteract then
						...
						if not Success then
							...
							print("Copybara | "..KeybindSettings.Name.." Callback Error " ..tostring(Response))
							wait(0.5)
							Keybind.Title.Text = KeybindSettings.Name
							TweenService:Create(Keybind, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
					...
					if Dragging then
						...
						if SliderSettings.CurrentValue ~= NewValue then
							...
							if not Success then
								...
								print("Copybara | "..SliderSettings.Name.." Callback Error " ..tostring(Response))
								wait(0.5)
								Slider.Title.Text = SliderSettings.Name
								TweenService:Create(Slider, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
	...
	if CEnabled then
		pcall(function()
			if isfile(ConfigurationFolder .. "/" .. CFileName .. ConfigurationExtension) then
				...
				Copybara:Notify({Title = "Configuration Loaded", Content = "The configuration file for this script has been loaded from a previous session"})
			end
		end)
	end
end

task.delay(3.5, Copybara.LoadConfiguration, Copybara)
